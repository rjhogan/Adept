...
main(int argc, char** argv)
{
  ...
  std::cout << "   Compiled with " << adept::compiler_version() << "\n";
  std::cout << "   Compiler flags \"" << adept::compiler_flags() 
	    << "\" (\"-g -O3\" is recommended)\n";
  std::cout << "   Jacobians processed in blocks of size " 
  ...
  for (int ialg = 0; ialg < N_TEST_ALGORITHMS; ialg++) {
	...
	if (use_tool[itool]) {
	  ...
	  if (rms_verify > tolerance) {
	    ...
	    std::cout << "      Jacobian agrees with hand-coding within tolerance of " << tolerance << "\n";
	  }
